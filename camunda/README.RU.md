...
### Настройка БД с помощью Dbmate
Чтобы безопасно включить слоты логической репликации CDC и схемы публикаций без изменения стандартных конфигураций Docker, мы используем [Dbmate](https://github.com/amacneil/dbmate).

Миграции автоматически применяются во время развертывания с помощью контейнера запуска `amacneil/dbmate:2.15.0`, который ожидает инициализации схемных таблиц Camunda (используя стандартную проверку таблиц через `psql`) перед настройкой репликации:
