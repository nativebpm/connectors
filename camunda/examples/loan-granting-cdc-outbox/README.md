...
## ⚡ Следующий эволюционный шаг: Wasmee Direct Push в NativeBPM

Хотя связка Postgres WAL + Sequin CDC решает проблему тяжелого REST-поллинга в Camunda 7, в архитектуре **NativeBPM** сделан следующий качественный шаг: переход на рантайм **[Wasmee](https://wasmee.com)**:
- **Без посредников**: Не требуются сторонние CDC-демоны, триггеры БД и Kafka-пайплайны.
- **Прямой Push (< 5 мкс)**: Задачи выталкиваются ядром движка напрямую в воркер со скоростью оперативной памяти.
- **Ambient Durability**: Автоматическое сохранение контрольных точек с гарантией возобновления ровно с прерванного шага при любых сбоях оборудования.
- **0% очередей в БД**: Полная разгрузка СУБД от очередей, локов и транзакционной фрагментации.
