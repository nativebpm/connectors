...
2. **Stateless-хост и надежное хранилище**: Узлы, выполняющие код, остаются полностью stateless. Все состояние процесса (снимки памяти, журнал вызовов oplog) сохраняется во внешнем S3-совместимом или оперативной памяти (in-memory) хранилище.
...
```
      ...
      ▼                     ▼
┌───────────┐         ┌───────────┐
│ guest_vm  │ (WASM)  │  worker   │ (WASM бизнес-логика)
│ (напр. DMN│         │  Воркер   │
└─────┬─────┘         └─────┬─────┘
      │                     │
      └──────────┬──────────┘
       ...
       └─────────┬────────┘
         ...
         ┌────────┴────────┐
         ▼                 ▼
    [ Хранилище S3 ]  [ Оперативная память ]
```

> [!NOTE]
> Основное ядро модуля **wasman** спроектировано как максимально легковесное и зависит только от `S3` (хранилище S3 Snapshot Store) и `wasman/runner` (SDK раннера на стороне гостевой VM). Все остальные модули в директории `examples/` (такие как `camunda`, `temporal` или `gotenberg-telegram`) являются исключительно демонстрационными интеграциями и не входят в число зависимостей основного движка выполнения.
...
- [memory_store.go](memory_store.go): Хранилище снимков в оперативной памяти с потокобезопасной блокировкой.
...
func main() {
	// 1. Инициализируем хранилище в памяти
